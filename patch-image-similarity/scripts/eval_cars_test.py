...
def load_cars_test(cars_dir):
    ...
    test_classes = [str(i) for i in range(99, 197)]  # standard CARS196 metric-learning test split
    images = []
    for c in test_classes:
        for path in by_class[c]:
            images.append({"class_id": c, "path": path})
    return images
...
def main():
    ...
    print(f"Loading CARS196 test set from {args.cars_dir}...")
    test_images = load_cars_test(args.cars_dir)
    ...
    emb_cache_path = os.path.join(os.path.dirname(args.checkpoint) if args.checkpoint != "zero-shot" else ".", "cars_embeddings_cache.pt")
    ...
    hits_1 = 0
    # NOTE: query_chunk=8 (not 32) -- a 32/4096 combo OOM'd on a 24GB GPU for
    # CUB's eval (same bug, same fix); keep this small regardless of gallery size.
    query_chunk_size = 8
    ...
    for c_start in tqdm(range(0, N, cand_chunk_size), desc="Candidate chunks"):
        ...
        c_embs = all_embeddings[c_start:c_end].to(device)

        for q_start in range(0, N, query_chunk_size):
            q_end = min(N, q_start + query_chunk_size)
            q_embs = all_embeddings[q_start:q_end].to(device)
            Q = q_end - q_start
    ...
    for i in range(N):
        if classes[i] == classes[best_idx[i]]:
            hits_1 += 1

    recall_1 = hits_1 / N
    print(f"\nFinal CARS196 Recall@1: {recall_1:.4f} ({hits_1}/{N})")

    out_path = os.path.join(os.path.dirname(args.checkpoint) if args.checkpoint != "zero-shot" else ".", "cars_test_recall.json")
    with open(out_path, "w") as f:
        json.dump({"test_size": N, "recall_1": recall_1, "hits": hits_1}, f, indent=2)
