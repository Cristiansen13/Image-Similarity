...
def main():
    ...
    print(f"Loading CUB test set from {args.cub_dir}...")
    ...
    emb_cache_path = os.path.join(os.path.dirname(args.checkpoint) if args.checkpoint != "zero-shot" else ".", "cub_embeddings_cache.pt")
    if os.path.exists(emb_cache_path):
        ...
        for i in tqdm(range(0, N, args.batch_size)):
            ...
            images = [Image.open(os.path.join(images_root, p)).convert("RGB").resize((IMAGE_SIZE, IMAGE_SIZE)) for p in batch_paths]
    ...
    recall_1 = hits_1 / N
    print(f"\nFinal CUB Recall@1: {recall_1:.4f} ({hits_1}/{N})")
    
    out_path = os.path.join(os.path.dirname(args.checkpoint) if args.checkpoint != "zero-shot" else ".", "cub_test_recall.json")
