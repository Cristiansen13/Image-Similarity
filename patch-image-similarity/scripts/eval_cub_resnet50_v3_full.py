"""Full-scale CUB Recall@1 eval for the ResNet50 v3 checkpoint (embedding
head + warmup/cosine schedule). Imports the exact model class from the
training script for checkpoint compatibility.

Usage: python eval_cub_resnet50_v3_full.py --cub-dir /path/CUB_200_2011 \
    --checkpoint checkpoints_cub_joint_resnet50_v3/model_final.pt --embed-dim 512
"""
import argparse
import json
import os
...
from finetune_cub_joint_resnet50_v3 import ResNet50Model, IMAGENET_MEAN, IMAGENET_STD, IMAGE_SIZE
...
def main():
    ...
    model = ResNet50Model(embed_dim=args.embed_dim, unfreeze_layer3=args.unfreeze_layer3).to(device).eval()
    ...
    all_global = torch.zeros((N, args.embed_dim), dtype=torch.float32)
    all_patches = torch.zeros((N, 49, args.embed_dim), dtype=torch.bfloat16)
    ...
    print("\n" + "=" * 70)
    print(f"FULL CUB TEST GALLERY (N={N}), ResNet50 v3 (embed head + warmup/cosine)")
    print("=" * 70)
    print("Published (BN-Inception): SoftTriple R@1=65.4%, MS R@1=65.7%, Proxy-Anchor R@1=71.1%")
    print("v1 untuned: 47.87%  v2 best (step1000): 49.39%")
    ...
    checkpoint_tag = os.path.basename(args.checkpoint).replace(".pt", "")
    out_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                             "data", f"cub_resnet50_v3_full_results_{checkpoint_tag}.json")
